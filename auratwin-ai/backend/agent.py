    ...
    def answer_query(
        ...
        if any(w in query for w in ["waste", "wasting", "inefficient", "highest power", "most power", "overcooled"]):
            ...
            if wasted_zones:
                ...
                response_text = (
                    ...
                    f"- **Occupancy:** `{worst_zone['estimated_occupancy']}` people (`{worst_zone['detected_people']}` detected by CCTV)\n"
                    f"- **Current Temperature / Setpoint:** `{worst_zone['temperature_c']}°C` / `{worst_zone['setpoint_c']}°C`\n"
                    f"- **Issue Detected:** {reason_str}\n\n"
                    f"**Recommended Action:** Apply the eco-mode recommendation to relax setpoint to "
        ...
        if any(w in query for w in ["save", "saving", "savings", "saved today", "cost", "tariff", "bill"]):
            response_text = (
                ...
                f"By synchronizing HVAC setpoints with current CCTV person counts, the building avoids conditioning empty classrooms and transit corridors."
            )
            return {
                "response": response_text,
        ...
        for z in zones_telemetry:
            if z["name"].lower() in query or z["zone_id"].lower() in query:
                ...
                response_text = (
                    ...
                    f"- **CCTV Snapshot:** `{z['detected_people']}` people detected ({z['occupancy_percentage']}% capacity)\n"
        ...
        if any(w in query for w in ["cluster", "kmeans", "k-means", "density", "zdi", "index"]):
            ...
            response_text = (
                ...
                f"The 5-dimensional feature vectors (`[detected people, temp, power, hour, occupancy ratio]`) classified the 10 building zones as follows:\n\n"
                ...
                f"K-Means dynamically feeds the Zone Density Index into the HVAC optimizer to adapt cooling priorities in real-time."
            )
            return {
                "response": response_text,
